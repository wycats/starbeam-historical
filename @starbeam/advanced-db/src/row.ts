  ...
  implements ModelInstance<T>
{
  ...
  static {
    debug(this, "Row").inspector((row, debug) =>
      debug.struct(
        {
        ...
        {
          description: `${row.#table.name}@${row.#id}`,
        }
      )
    );
  }
  ...
  private constructor(table: Table<any, T>, id: Id, columns: T["columns"]) {
    this.#table = table;
    this.#id = id;
    this.#columns = columns;
  }

  readonly [ROW]: Row<T> = this;
